...
\begin{frame}{Concept Check: Suggested Answers}
  \begin{enumerate}
    \item $\frac{7!}{3!2!2!}=210$ distinct sequences.
    \item Choosing the three P requests, then two of the remainder for S,
      leaves two for A: $\binom{7}{3}\binom{4}{2}=210$. This equals the
      multinomial coefficient $\binom{7}{3,2,2}$.
    \item Stars and bars gives
      $\binom{8+3-1}{3-1}=\binom{10}{2}=45$ profiles.
    \item No. Labeled routing sequences collapse into occupancy profiles
      with different multiplicities, so the profiles generally have
      different probabilities.
  \end{enumerate}

  \begin{block}{Instructor note}
    Require students to define the final outcome and identify which objects
    and categories are labeled before accepting a formula.
  \end{block}
\end{frame}
