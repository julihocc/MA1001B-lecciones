...
\begin{frame}{Concept Check: Suggested Answers}
  \begin{enumerate}
    \item The product rule gives $4\times3\times2=24$ complete configurations.
    \item The two roles are distinct, so $P(5,2)=5\times4=20$ assignments.
    \item Internal order does not change the panel, so $\binom{5}{2}=10$
      panels.
    \item A valid count cannot exceed the cardinality of its universe. The
      expression 420 therefore counts at least some final groups more than
      once.
  \end{enumerate}

  \begin{block}{Instructor note}
    Ask students to describe one outcome before naming a formula. For the
    final item, require them to identify repeated designation of members as
    the source of overcounting.
  \end{block}
\end{frame}
